...
export function DiscoverScreen() {
  ...
  const scrollToCombo = (id: string) => {
    comboRefs.current[id]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  };

  return (
    <div className="screen">
      <StatusBar />
      <div className="screen__body" style={{ padding: '0 var(--screen-x)' }}>
        ...
        {[...combos]
          ...
          .map((combo) => {
            ...
            return (
              <div
                key={combo.id}
                ref={(el) => { comboRefs.current[combo.id] = el; }}
                style={{ background: 'var(--bg-s)', borderRadius: 16, padding: 16, border: '1px solid var(--bd)', marginBottom: 12 }}
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  ...
                  <button
                    ...
                    aria-label={upvoted ? 'Remove upvote' : 'Upvote combo'}
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" className="btn-secondary" style={{ width: 'auto', padding: '14px 16px' }} onClick={() => navigate(`/sigil/community/${combo.id}`)} aria-label="Generate sigil">
                    ◈
                  </button>
                </div>
              </div>
