...
export function SessionScreen() {
  ...
  return (
    <div
      className="screen"
      style={{ background: `radial-gradient(circle at center, ${word.color}22 0%, #0F0905 65%)`, userSelect: 'none' }}
    >
      ...
      <div
        style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px', gap: 18, cursor: 'pointer' }}
        onClick={handleTap}
        onKeyDown={(e) => e.key === ' ' && handleTap()}
        role="button"
        tabIndex={0}
        aria-label={done ? 'Session complete' : `Tap to count repetitions, ${sessionCount} of ${target}`}
      >
        <div style={{ position: 'relative', width: 260, height: 260, flexShrink: 0, pointerEvents: 'none' }}>
        ...
        {!done && (
          <button
            ...
            style={{ marginTop: 8, padding: '10px 20px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 20, color: 'rgba(255,248,242,0.6)', fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font-body)' }}
          >
